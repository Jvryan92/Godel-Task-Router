...
export async function handlePullRequest(env: Env, payload: WebhookPayload): Promise<void> {
  const installationId = payload.installation?.id;
  const repo = payload.repository;
  const pr = payload.pull_request;

  if (!installationId || !repo || !pr) {
    console.log('Missing required PR payload fields');
    return;
  }
  ...
  console.log(`Processing PR #${pr.number} to ${repo.full_name} @ ${sha.substring(0, 7)}`);
  ...
  // Create check run for the PR head SHA
  ...
  try {
    ...
    console.log(`PR check completed: score ${result.integrityScore}/100`);

  } catch (error) {
    console.error('Error running PR integrity check:', error);
...
async function runIntegrityCheck(
  ...
  // Fetch repository files at PR head
